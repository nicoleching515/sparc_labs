...
/* ─── ✎ PUBLICATIONS ─────────────────────────────────────────────────────
   Moved to data.jsx so the homepage and publications.html share one list.
   Edit PUBS there; this page shows the first HOME_LIMIT (5) entries.    */
...
/* ─── ✎ SPEAKER SERIES ────────────────────────────────────────────────
   The sheet loader (EVENTS_SHEET_ID + useSheetEvents) lives in data.jsx
   so the homepage and events.html share it. Homepage shows first 5.   */
    ...
    <section className="section" id="publications">
      <div className="container">
        ...
        <div className="pubs reveal">
          {PUBS.slice(0, HOME_LIMIT).map((p, i) => <PubRow key={i} p={p} />)}
        </div>
        {PUBS.length > HOME_LIMIT ?
        <a className="list-more reveal" href="publications.html">
          View all {PUBS.length} publications <span className="arrow">→</span>
        </a> :
        <a className="list-more reveal" href="publications.html">
          View all publications <span className="arrow">→</span>
        </a>}
        <EditNote>✎ Edit in data.jsx → PUBS[] · add each paper's link in the `url` field</EditNote>
      </div>
    </section>);
    ...
    <section className="section" id="events">
      <div className="container">
        ...
        <div className="events-list reveal">
          {list.slice(0, HOME_LIMIT).map((e, i) => <EventRow key={i} e={e} />)}
        </div> :
        <div className="list-empty reveal">Schedule to be confirmed!</div>}
        {list.length > HOME_LIMIT ?
        <a className="list-more reveal" href="events.html">
          View all {list.length} talks <span className="arrow">→</span>
        </a> :
        <a className="list-more reveal" href="events.html">
          View the full schedule <span className="arrow">→</span>
        </a>}
